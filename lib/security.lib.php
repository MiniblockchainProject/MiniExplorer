...
function clean_form_input($input, $maxlen=99) {
  foreach ($input as $field_name => $value) {
    if (is_array($value)) {
	  foreach ($value as $mf_name => $mf_value) {
		$value[$mf_name] = clean_string_input($mf_value);
		if (strlen($value[$mf_name]) > $maxlen) {
		  die('error: data submitted was excessively large');
		}
	  }
	} else {
	  $input[$field_name] = clean_string_input($value);
	  if (strlen($input[$field_name]) > $maxlen) {
		die('error: data submitted was excessively large');
	  }
    }
  }
...
function check_email_dns($email_str) {
  @list($userName, $mailDomain) = explode('@', $email_str); 
...
function check_pass_strength($pwd, $min=10, $max=99) {
