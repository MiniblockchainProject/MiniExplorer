...
function float_format($num, $dec=10, $pow=0, $sep='.', $ths='') {
  return number_format(bcmul(remove_ep($num), pow(10, $pow)), $dec-$pow, $sep, $ths);	
}

function pretty_format($num, $dec=10, $pow=0, $sep='.', $ths=',') {
  return number_format(bcmul(remove_ep($num), pow(10, $pow)), $dec-$pow, $sep, $ths);	
}
...
function int_to_coins($num, $coin=1e10) {
  return bcdiv(trim($num), $coin);
}
